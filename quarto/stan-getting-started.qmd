...
## A.10 Stochastic processes and Markov chains {.unnumbered}
...
- \theta$. If its expected error is zero, an estimator is _unbiased_.
If the Monte Carlo draws $\theta^{(1)}, \ldots, \theta^{(M)}$ are
independent, the _central limit theorem_ tells us that our error
follows a normal distribution asymptotically, so that as $M
\rightarrow \infty$, the error of our estimate $\widehat{\theta}$
