...
varaibles, which can be shown to be equal to the log posterior plus a
constant.  
...
In the case here, our sampling distribution is symmetric, so we do not
need to add sampling uncertainty if our only goal is to estimate an
expectation.\footnote{The *Rao-Blackwell theorem* tells us that using
expectations is at least as efficient as samples; in practice it can
