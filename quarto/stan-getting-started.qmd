...
I'd like to thank Mitzi Morris, Barry Smith, Brian Ward, and Jeff
Soules for helpful feedback on earlier drafts. Thanks to Ero Carrera
for typo fixes.
...
is $\sqrt{D} = \sqrt{\underbrace{1 + 1 + \cdots 1}_{d \
\textrm{times}}}$ in $D$ dimensions.  Now let's see what happens to
the proportion of volume in the inscribed hypersphere as the
...
A relation $R \subseteq A \times A$ maybe be *closed* under relations
...
Suppose we have two different measures, $\Pr$ and $\textrm{Qr}$ over the
same $\sigma$-algebra $(\Omega, \mathcal{S}).$  The *total variation
distance* between $\Pr$ and $\textrm{Qr}$ is written as $\left|\left|
\strut \Pr - \textrm{Qr} \right|\right|$ and defined by 
$$
\left|\left| \strut \Pr - \textrm{Qr} \right|\right|
= \max_{A \in \mathcal{S}} \left| \strut \Pr[A] - \textrm{Qr}[A] \right|.
$$
