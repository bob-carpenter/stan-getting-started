...
Now we can fit Laplace's model by compling the model, constructing a
dictionary for the data, then calling the `sample` method on the
...
With approximate draws from the posterior, we can plot a histogram of
$\theta$ to give us a sense of our uncertainty in its value.
...
All of the draws have a value for $\theta$ that is greater than 0.5.
In the next sections, we will see how to turn that into a probability
estimate.


## Bayesian point estimates

We can average the posterior draws to produce a _Bayesian point
estimate_ for $\theta$,
\begin{align}
\widehat{\theta}
&= \mathbb{E}[\theta \mid y]
...
The posterior mean of the draws for `theta` is our estimate for $\theta$.
...
We see that our point estimate is right in the middle of our
histogram, as we would expect from it being an average. Because it
involves sampling from the posterior, Monte Carlo estimation is a
_stochastic algorithm_, which means it can return a different result
each time it is run (assuming we change the pseudorandom number seed,
which completely determines the chain of random numbers produced by a
pseudorandom number generator). Stochastic algorithms can Stochastic
algorithms are sometimes called _Monte Carlo algorithms_ in a nod
toward the famous casino of the same name.
...
$$
Events are what are assigned probabilities by measures in probability
theory. Given a probability model, the probability of the event $A$,
...
to write $\Pr[\theta > \frac{1}{2} \mid N, y]$ for the probabilty of
the event $\theta > \frac{1}{2}$.
...
\\[8pt]
&\approx \frac{1}{M} \sum_{m=1}^M \textrm{I}(\theta^{(m)}),
\end{align}
...
the indicator function to a random variable and the notaton
$\textrm{I}(\cdot)$ when we apply it to a primitive scalar.

Event probabilities can be calculated as the expectation of an
indicator function for the event. The indicator function takes on
value 1 if the parameters are in the event and value 0 otherwise.
The indicator is defined in the generated quantities block as
```stan
generated quantities {
  int<lower=0, upper=1> boys_gt_girls = theta > 0.5;
}  
```
In Stan, conditional expressions like `theta > 0.5` take on the value
1 if they are true and 0 if they are false---the indicator function itself
isn't written.  The posterior mean of `boys_gt_girls` is our estimate
for $\Pr[\theta > 0.5 \mid N, y]$.  
...
The value of 1.0 returned as an estimate brings up the important
...
$$
\Pr[\theta > \frac{1}{2}] \approx 1 - 10^{-26.87117}.
$$
That is, we'd need on the order of $10^27$ draws before we would see a
value of $\theta$ less than $\frac{1}{2}$.  As given, the answer of 1.0 is
very close to the true answer and well within our expected Monte Carlo
error.
