...
The anti-correlated chain converges much more quickly and more stably
than the independent chain, which in turn converges much more quickly
than the correlated chain.
...
We start with two-unit square centered at the origin, with values
represented as pairs in $[-1, 1]^2.$  Then we will generate points
uniformly at random in this square.  For each point, we will
calculate whether it falls inside the unit circle circumscribed
within the square.  The proportion of such points gives the proportion
of the square's volume taken up by the circle.  Because the square is
$2 \times 2$, it has an area of 4, so the circle has an area of 4 times the
proportion of points falling in it.
...
The mean of the draws for `pi` will be our estimate of $\pi$.
...
inside the circle.  We could have just multiplied this on the outside by 4, but instead we can just take the mean of the draws for `pi` to get our estimate for $\pi$.
