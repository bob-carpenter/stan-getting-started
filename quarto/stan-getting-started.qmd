...
reasoning under uncertainty from limited and typically noisy
observations.  We do that to draw inferences about a population after
measuring a subset of its members and to draw inferences about future
events after observing past events.
...
Bayesian inference is always done using a mathematical model model of
a stochastic data generating process.  If the model is well-specified
in the sense of matching the true data generating process, then
Bayesian statistical inference can be shown to have several desirable
properties, such as calibration and lack of overfitting.
...
We include the following Python boilerplate to import and configure packages we will use throughout.
