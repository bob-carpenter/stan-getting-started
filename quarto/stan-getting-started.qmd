...
the posterior density of the restaurants' probabiltiy of receiving a
...
defintions.   is executed once, after the data is read is
...
deifned. It contains only variabl declarations with sizes. When
...
derivatives fromn the log density value down to the unconstrained
...
that result (multipliation binds more tightly than addition and as in
...
setosa*'s scale, as long we can build separate regresisons for each.
...
```
which tells the number of columsn of `foo[idxs]` is `N` and
...
_missing data_ to finesse a philosophical aversion to probabilty
...
ieration.  In general, local variables can be assigned and reassigned
...
all parameters (simulted values were $\lambda = 0.51$, $\mu = [162 \ 175]$
...
## Support over constrained parameters

Stan requires programs to assign a finite log density to any
parameters that satisfy the constraints.  Therefore, we should not be
writing Stan programs with a mismatch between the parameter
constraints and the model constraints.  For example, the following
program puts not constraints on `sigma` in the declaration, but the
...
```
The way to fix this is to declare `sigma` to be of type
`real<lower=0>`.  Often this kind of bug shows up as a failure to
randomly initialize.  For instance, if we had `vector[10] sigma`, we'd
only have a $2^{-10}$ or about 1/1000 chance of generating valid
initializations at random, because Stan initializes parameters
using a $\textrm{uniform}(-2, 2)$ distribution on the unconstrained scale.
...
becomes $-\infty$ (and thus causes rejections).
