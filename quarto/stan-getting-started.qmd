...
helpful feedback on earlier drafts. Thanks to ChatGPT 4.0 (May 3, 2023
version) for extensive help on plotting and data frame construction.
I'd also like to thank the entire Stan Development Team and all of our
users.  Stan is an open project and a large team effort and we're
always happy to help onboard new developers, and we wouldn't be doing
it without users.
...
A _sample space_ is a set $\Omega$, the members of which represent
possible ways the world might be (i.e., what are sometimes known as
"possible worlds").
...
*Example 1*: The power set of any set forms a $\sigma$-algebra set
because it contains the empty set and is closed under complements and
arbitrary unions.

*Example 2*: A second simple example can be defined over the real
numbers $\mathbb{R}$ by taking the minimu set including all of the
open intervals $(a, b) \subset \mathbb{R}$ and closed under
complements and countable unions.  The resulting $\sigma$-algebra
defined from closing open sets this way is called a *Borel algebra*.
...
## A.2 Random variables, cdfs, pdfs, and pmfs {.unnumbered}
...
## A.3 Expectation notation {.unnumbered}
...
## A.4 Quantiles, medians, and uncertainty intervals {.unnumbered}
...
## B.4 Bayesian inference {.unnumbered}
...
# C. Installed packages {.unnumbered}
