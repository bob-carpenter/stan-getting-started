...
Defining a function with the suffix `_rng` will define a random number
...
```stan
  for (n in 1:N) {
    petal_length[n] ~ normal(alpha + beta * petal_width[n],
                             sigma);
  }  
```
...
width on length for *iris setosa* $\widehat{\beta}_1 = 0.075$, where
there is a large effect for *iris * of $\widehat{\beta}_2 = 0.6$.
Furthermore, our value for $\sigma$ is lower, which indicates lower
residual error in our predictors.  We will leave evaluating varying
error scales to the reader.
