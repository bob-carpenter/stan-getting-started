...
We can fit the model in Python as we did before and print the first 100 values simulated for the Markov chain $y$.
...
Next, we will show a running average of the 0 and 1 draws for the three chains.  
...
if the condition `cond` is rue and `e2` otherwise.
...
functions to convert back and forth for common cases, but complicated
cases must be handled manually (but remember, loops that just reassign
are fast in Stan).
...
Bayes formulated the _inverse problem_ of determining a posterior
distribution with density $p(\theta \mid y)$ from a prior with density
$p(\theta)$, a sampling distribution with density $p(y \mid \theta)$,
and observed data $y$.  But he was not able to solve the integral
presented in the denominator of his theorem and actually determine the
...
The posterior mean is not an arbitrary choice.  It can be shown that
the posterior mean minimizes expected squared error, which is the loss
defined as the squared difference between the estimate and the true value.
...
Posterior medians can be shown to minimize expected absolute error
(the loss being the absolute difference between the estimate and the
true value).
