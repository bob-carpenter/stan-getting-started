...
A process such as this is frought with *measurement error* of weights
...
the highest probabilty of their next review being a 5-star one.  But
...
zero and then incremented throughougout the execution of a Stan
...
those expections.  The sense in which it is better is that it will
...
simulations of 100 draws each for aestimators based on simulating from
...
graph and hence much more work when updating the chain rule for
derivatives.  The vectorized form has a single output variable for the
log density with three edges leading to `alpha`, `beta`, and `sigma.`
It will also be clever enough to calculate `log(sigma)` once and reuse
it.  This advantage is even bigger for distributions like multivariate
normal that require expensive internal determinant calculations.
