...
: X(\omega) \leq x \} \in \mathcal{S}$ (i.e., it is measurable).
...
has a range $(0.7, 0.9)$, whereas it is $(0.79, 0.81)$ on tyhe rig.
...
mydraw(pn.ggplot(df, pn.aes(x = 'u'))
...
### Stationarity-preserving transition distributions

In applications of Markov chains to statistical inference, we
typically have a target density (e.g., a Bayesian posterior density)
and seek to create a transition function that preserves it.
Suppose we have a Markov chain $X_0, X_1, \ldots.$
The transition probability mass or density function $p_{X_{n + 1} \mid
X_n}$ *preserves* the target density $\pi$ if $p_{X_0}(x_0) = \pi(x_0)$ implies
$X_0, X_1, \ldots$ is a stationary Markov chain.  If $\pi$ is
preserved, stationarity implies the chain has $p_{X_n}(x_n) =
\pi(x_n)$ for all $n \in \mathbb{N}.$  A chain whose transition
distribution preserves $\pi$ will only be stationary if initialiozed
from the stationary distribution (i.e., $p_{X_0} = \pi$).
...
have a probability measure $\phi$ over the usual Borel algebra for
$\mathbb{R}^N.$  In the standard definition, $\phi$ can be an arbitrary
measure over a general state space.  Note that the measure $\phi$ is
distinct from the measure $\Pr$ defining the Markov chain, though it
is often taken to be the stationary distribution defined by the chain
if it is stationary.

A Markov chain $X = X_0, X_1, \ldots$ is $\phi$-irreducible if for every $A
\subseteq \mathbb{R}^N$ such that $\varphi[A] > 0,$ and for every point $x
\in \mathbb{R}^N,$ there exists an $m \in \mathbb{N}$ such that
$$
\varphi\left[X_{m + n} \in A \mid X_n = x\right] > 0.
$$
...
# C.  Monte Carlo and Markov chain Monte Carlo methods {.unnumbered}
