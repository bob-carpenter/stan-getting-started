...
randomness from an underlying probabiltiy measure that determines how
...
compatible with these langauges' built-in Bayesian analysis tools as
well as Stan's own tools, some of which we will cover in this
introduction.  Stan's also avaialbel in Mathematica, Stata, and
...
warnings and informatial messages that would otherwise provide updates
...
the data givein in the Python argument `data` to a file, then reads in
...
misses, it goes to system RAM, which sabout twice as slow as fetching
...
```{python}
n_base = 512 if DRAFT else 2048
ns = [n_base, n_base * 4, n_base * 16]:
print("Python (1 repetition):")
for n in [n_base, n_base * 4, n_base * 16]:
    ...
    for j in range(n):
        for i in range(n):
            _ = A[i, j]
    print(f"column major: n = {n:>5};  {(time.time() - start_time):6.2f} seconds")
```
...
```{.stan include="../stan/row-major-vs-column-major.stan" filename="stan/row-major-vs-column-major.stan"}
```

We can then run the comparison in Stan.  We had to set a
value in the loop, otherwise the C++ compiler will realize the whole
loop is a no-op and optimize it out of existence.
```{python}
model = csp.CmdStanModel(stan_file = "../stan/row-major-vs-column-major.stan")
print("Stan results:")
for n in ns:
    matrix = np.random.rand(n, n)
    start_time = time.time()
    fit = model.sample({'N': n, 'row_major': 1, 'A': matrix},
                       iter_warmup=0, iter_sampling=1, seed=123,
                       show_progress = False, show_console = False)
    print(f"   row major: n = {n:>5};  {(time.time() - start_time):7.3f} seconds")
    start_time = time.time()
    fit = model.sample({'N': n, 'row_major': 0, 'A': matrix},
                       iter_warmup=0, iter_sampling=10, seed=123,
                       show_progress = False, show_console = False)
    print(f"   col major: n = {n:>5};  {(time.time() - start_time):7.3f} seconds")
```
...
int m = y;   // ILLEGAL! cannot demote real to ingeger
```
...
derivative calculations; see the sectiosn of this document on [automatic
...
predictors, independent variables) to a regression.  For instance if I
...
residence are unmodeled covariates and the vote is modeled data if it
is known.
...
success $\theta \in (0, 1)$, and number of successes $y \in \{ 0, 1,
\ldots N \}.$
...
pass and multiply/adds for the chain rule in the backward pass.
...
1, \ldots, n \}$ to $A.$ The number $n$ is the *size* of the set.  A
set is *infinite* if it is not finite.

A set $A$ is *countable* if there is surjection from the counting
numbers $\mathbb{N}$ to $A$ and is *uncountable* otherwise.  A
countable set may be either finite or infinite.  It can be shown that
the integers $\mathbb{Z}$ are countable and the set of real numbers
