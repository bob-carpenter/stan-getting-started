...
Cauchy distribution, with $p_Y(y) = \frac{1}{\pi} \cdot \frac{1}{1 +
y^2}.$ The fundamental problem is that $p_Y(y)$ does not converge to
zero quickly enough as $y \rightarrow \infty$ or $y \rightarrow
-\infty$.  Contrast the Cauchy densit with the normal density,
$p_{X_1}(x) \propto \exp\!\left(-\frac{1}{2}\cdot
x\right),$ which converges exponentially fast.
...
## A.10 The central limit theorem

The *central limit theorem* provides a profound insight into the
behavior of averages of independent random variables and explains the
centrality of the normal distribution in statistics.  Suppose that we
have an infinite sequence of random variables $X_0, X_1, \ldots$ that
are *independent and identically distributed* (i.i.d.) with finite
mean and variance (i.e., $\mathbb{E}[X_n]$ and $\textrm{var}[X_n]$
exist.
