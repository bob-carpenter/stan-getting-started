...
I would strongly recommend one of our more recent interfaces that
invoke Stan's command-line interface in a separate process.
...
The process they call is the Stan command-line interface (CLI).

* [CmdStan](https://mc-stan.org/users/interfaces/cmdstan): CLI

This makes them relatively easy to install because there are no C++
compatibility issues to work out between the analysis languages.
...
The first two interfaces developed for Stan were for R and Python.

* R: [RStan](http://mc-stan.org/rstan/)
* Python: [PyStan](https://pystan.readthedocs.io/en/latest/index.html)

More recently, the Python back end was replaced with a web service.

* HTTP: [HttpStan](https://github.com/stan-dev/httpstan)

All three of these interfaces use high-level, foreign-function interfaces
(FFI) to Stan's C++ (Cython in Python, Rcpp in R), which complicates
installation, especially on Windows.  None of them are as fast as the
out-of-process interfaces (unless you have a very very slow file
system) and all of them consume more memory.
...
We can see that the data have been quantized to the millimeter.  We
can also see that the three species have highly variable sizes, but
a roughly linearly relation between petal width and petal length.  

We can now write a Stan model to regress petal length on the petal width.

```{.stan filename='../stan/iris-petals.stan'}
data {
  int<lower=0> N;
  array[N] real<lower=0> petal_width;
  array[N] real<lower=0> petal_length;
}
parameters {
  real alpha;
  real beta;
...
This summary doesn't give us a good feeling for the uncertainty in the linear relationship.
We could plot the line determined by $\widehat{\alpha}, \widehat{\beta}$ along with the
uncertainty determined by $\widehat{\sigma}$, but we would be missing the characterization of
the other possible values of $\alpha, \beta, \sigma$ that are consistent with our data.
...
data {
  int<lower=0> N_tilde;
  vector<lower=0>[N_tilde] petal_width;
}
parameters {
  real alpha;
  real beta;
...
generated quantities {
  vector<lower=0>[N_tilde] petal_length_sim
    = normal_rng(alpha + beta * petal_width, sigma);
  vector<lower=0>[N_tilde] petal_length_hat
    = alpha + beta * petal_width;
}
```

It is important that we have used _exactly the same parameters_ as in the original model.
Fitting generated quantities, we will read that fit back in and use those draws
to run the new generated quantities block, which will do our predictions for new data.
