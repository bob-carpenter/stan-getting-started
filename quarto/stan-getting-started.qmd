...
If you're looking for a gentle introduction to Bayesian statistics,
I can highly recommend \textit{Statistical Rethinking} @mcelreath2023.
...
wide range of statistical models to be coded.  At its heart, a Stan
program declares data variables, parameters, and defines a posterior
log density up to a constant.  The most straightforward way to define
the posterior log density up to a constant is to define the joint
density and apply Bayes's rule, but Stan allows intermediate optimizations that lead to 
...
functions, expectations, and the basic rules of probability theory
(e.g., as taught in an upper level undergraduate introduction to
...
* CLI: [CmdStan](https://mc-stan.org/users/interfaces/cmdstan)
...
* Python: [CmdStanPy](https://mc-stan.org/cmdstanpy/)
* R: [CmdStanR](https://https://mc-stan.org/cmdstanr/)
* Julia: [Stan.jl](https://mc-stan.org/users/interfaces/julia-stan)
...
* R: [RStan](http://mc-stan.org/rstan/)
* Python: [PyStan](https://pystan.readthedocs.io/en/latest/index.html)

The Python in-memory interface was later replaced with the following
HTTP-based web service.

* HTTP: [HttpStan](https://github.com/stan-dev/httpstan)
...
* R, Python, Julia, C, Rust: [BridgeStan](https://github.com/roualdes/bridgestan)
...
expressions are coded as 1 (true) or 0 (false) in Stan, like in C++.
Because `y[m - 1]` is constrained to take on values 0 or 1, we know
that `y[m - 1] == 1` is equivalent to `y[m - 1]`.  Second, it uses the
_ternary operator_, also like in C++.  The expression `cond ? e1 : e2`
involves three arguments, separated by a question mark (`?`) and a
colon (`:`); it's value is the value of `e1` if `cond` is true and the
value of `e2` otherwise.  Unlike an ordinary function, the ternary
operator only evaluates `e1` if the condition is true and only
evaluates `e2` if the condition is false.
...
We then set a variabler `pi` equal to 4 times the indicator value.
...
# Stan for solving inverse problems
...
Bayes's rule presents an integral in the denominator that is often
intractable in applications.  The good news is that most applications
do not require us to solve this integral.  The bad news is that they
require estimating expectations, which are the solution to a different
intractable integral over the posterior.  The good news that makes
modern Bayesian statistics possible is that we can solve these
intractable integrals using Markov chain Monte Carlo methods.
...
distributed data.  A _conjugate prior_ for a sampling distribution is
a family of distributions such that if the prior is drawn from that
family, the posterior will be, too.  Laplace's so-called _rule of
succession_ is a special case of this relationship, with a conjugate
beta prior for binomial data.  We will work through Laplace's analysis
in what has become the standard textbook introduction to Bayesian
inference. 
...
being identically drawn from the posterior. In well behaved models,
with a modestly large $M$ (hundreds or thousands), the draws will
become numerically indistinguishable from true draws from the
posterior.
...
weights given by a probability density.  Bayesian inference invariably
comes down to estimating expectations over the posterior density of a
Bayesian model.  The concise notation for this is conditional
...
(MAP) estimator. Posterior modes can be calculated with Stan using optimization.
...
The posterior median has three pleasant properties. First, it is always
well defined, even for densities with poles or very wide tails.
Second, it is more robust to outliers than squared error because
errors are not amplified by squaring.  Third, the posterior median
estimator minimizes expected absolute error.
...
Stan performs a number of _warmup iterations_, during which it
...
diagonal, i.e., the variances, by default).  The step size is used in
the Hamiltonian dynamics simulator and the (co)variance is used as a
_preconditioner_.  A preconditioner warps the sample space, in our
case, with a fixed _Euclidean metric_, which Stan estimates using the
covariance of roughly the last half of the warmup iterations.
...
argument of the `sampling()` method and you can set the maximum number
...
and La Delicias Mexianas in Spanish Harlem.  Here's the number of
...
Variables declared in the parameters block may never be set---they are
provided externally by the inference algorithm (or the user in the
case of BridgeStan).  After the transformed parameters block has
executed, variables declared in it may not be reassigned.
...
parameters.  The notation `lpdf` is used for log probability density functions,
`lpmf` for log probability mass functions. The variants `lupdf` and
`lupmf` are
for their unnormalized counterparts that may drop normalizing constants.
Unless the normalizing constants are needed, for example in a mixture
model component, it is more efficient to use the `lupdf` and `lupmf`
forms  either explicitly or implicitly using sampling statements.
...
functions are called using standard vertical bar notation to separate
...
They may also be used directly in sampling statements after removing
the suffix.
```stan
  z ~ foo(phi);
```
...
$\alpha$ and slope $\beta$.  A linear regression allows an inexact
relationship where the linear relation only holds in expectation.  In
real observations, there will be _error_ due to either measurement or
a failure of the linear relationship.  
...
$\tilde{y}$, we can take posterior means of these values.  In the case
here, our sampling distribution is symmetric, so we do not need to add
sampling uncertainty if our only goal is to estimate an expectation.
In general, using expectations in place of draws is more efficient,
as shown by the Rao-Blackwell theorem; replacing draws with
expectations is sometimes called *Rao-Blackwellization*.
...
Stan's \textit{standalone generated quantities} feature that lets us
...
is \textit{contravariant}, so that if a type `T` is assignable to a
type `U`, then a type `array[N] T` is assignable to `array[N] U`.
Similarly, integer-valued primitives and containers may be assigned to
their real or complex counterparts, and real-valued primitives and
containers may be assigned to their complex counterparts (but not the
other way around).  Similarly, variables of a constrained type may be
assigned to variables of an unconstrained type.
...
$$
This is called the \textit{complete data likelihood} in frequentist
settings. 
...
a $K$-vector and then applies useless autodiff steps in the reverse
pass. 
```stan
data {
  real<lower=0> alpha;
...
```
This is correct and will provide the right answer, but it is inefficient.
In situations like this, we can move `y_tilde` down to generated
quantities because it's conditionally independent of the data
given `mu` and `sigma.`
```stan
parameters {
  real alpha, beta;
...
is relatively efficient for large `M`, but slicing out a column using
...
The paper that kicked off the focus on workflow is @gabry2019; the
current state of our thinking is @gelman2020workflow.
