...
An injective function is called an *injection,* a surjective one a
*surjection,* and a bijective one a *bijection.* also said to be
"onto."  A function that is not injective is said to be *many to one*.
...
Because $\Omega \in \mathcal{S}$, its complement $\emptyset =
...
$$
which extends to the countably infinite case
$$
...
defined by
$$
u = \textrm{logit}(x) \qquad v = \textrm{logit}(y \cdot (1 - x)).
$$
