...
For MCMC, there are two components to efficiency, the _statistical
efficiency_ of the sampler, which is measured by effective sample size
per iteration, and _program efficiency_, which is how fast the log
density function evaluates.

## Compuational efficiency

The main bottleneck for Stan's computation is the evaluation of the
log density and its gradient, which it does once per leapfrog step,
which means multiple times per draw (how many depends on how poorly
...
the transofrmed parameter `alpha`, so that `alpha` has a `normal(0,
