...
$2 x 2$, it has an area of 4, so the circle has an area of 4 times the
...
inside the circle.  We could have just multiplied ths on the outside by 4, but instead we can just take the mean of the draws for `pi` to get our estimate for $\pi$.
...
The posterior median has three plasant properties. First, it is always
...
increment thetarget log density, 
```stan
  target += foo_lpdf(z | phi);
```

They may also be used directly in sampling statments after removing
the suffix.
```stan
  z ~ foo(phi);
```
...
The parameters are marginalized out.  It's perhaps easiest to
understand by considering the Monte Carlo approximation, which just
averages the sampling log density over draws of parameters from the posterior.
...
the transformed data block. The difference is that we now use a
lognormal regression on log petal width instead of a linear regression
of length on width.
