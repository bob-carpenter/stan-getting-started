...
## Random variables, cdfs, pdfs, and pmfs {.unnumbered}
...
mydraw(
    ...
    pn.labs(x='x', y='value', title='pdf and cdf of X ~ normal(0, 1)') +
...
F_X(x) = 1 - F_X(-x).
$$
The function $1 - F_X(y)$ is known as the _complementary cumulative
...
# Create the plot
plot = (pn.ggplot(df, pn.aes(x='n', y='cdf')) +
    ...
    pn.scale_x_continuous(limits=(1,13),
			  ...
			  expand=(0, 0)))
for n in range(0,12):
    plot += pn.geom_segment(pn.aes(x = n + 1,
                                   y = cdf_values[n],
                                   xend = n + 2 - 0.075,
                                   yend = cdf_values[n]))
plot += pn.geom_point(pn.aes(x='n', y='cdf_low'),
...
## B.2 Expectation notation {.unnumbered}
...
## B.3 Quantiles, medians, and uncertainty intervals {.unnumbered}
