...
helpful feedback on earlier drafts.  Thanks to Ero Carrera for typo
fixes.

Thanks to [ChatGPT Plus
4.0](https://en.wikipedia.org/wiki/GPT-4) (May 5, 12, 24, 2023
versions) for extensive help on plotting and data frame construction
and rewriting some passages I couldn't manage to write clearly. I'd
also like to thank the entire Stan Development Team and all of our
users.  Stan is an open project and a large team effort and we're
always happy to help onboard new developers and new users.
...
like the standard Cauchy distribution.
...
# AA.  Set theory {.unnumbered}

Probability theory is based on simple notions from set theory such as
unions and intersections.  We will need to generalize to infinite sets
...
$$
Another common notation for power sets is $\mathbf{2}^A.$
...
*De Morgan's law* relate complementation, intersection, and union,
...
b)$ for the *tuple* consisting of the pair $a$ and $b$ in that order.
The identity conditions for tuples are that $(a, b) = (a', b')$ if and
...
$$
Despite the nesting, we write $(a, b, c)$ rather than $(a, (b, c)).$
...
\times B.$ For example, the comparison operator $\leq$ defines a
relation over the real numbers $L = \{ (x, y) \in \mathbb{R} \times
\mathbb{R} : x \leq y \}.$
...
defined by
$$
R \circ T = \{ (a, c) : (a, b) \in R, (b, c) \in R \}.
$$
...
\rightarrow A$ defined so that $f^{-1}(b) = a$ if $f(a) = b.$  The
composition of a function with its inverse or vice-versa is the
identity function, $f \circ f^{-} = \textrm{id},$ $f^{-} \circ f =
\textrm{id}.$  Thus for a bijection $f:A \rightarrow B,$ we have
$f^{-1}(f(a)) = a$ for all $a \in A$ and $f(f^{-}(b)) = b$ for all $b
\in B.$
...
## A.11 Finite, countable, and uncountable sets {.unnumbered}

A set $A$ is *finite* if there exists a number $n \in
\mathbb{N}$ and a bijection from $\{ 0, 1, \ldots, n \}$ to $A.$
The number $n$ is the *size* of the set.  A set is *infinite* if it is
not finite.

A set $A$ is *countable* if there is surjection from $\mathbb{N}$ to
$A$ and is *uncountable* otherwise.  A countable set may be either
finite or infinite.  It can be shown that the integers $\mathbb{Z}$
are countable and the set of real numbers $\mathbb{R}$ is uncountable.
...
<div class="example-text">
*Example 1(b)*: An example of a countably infinite sample space, we can
...
is different geometrically than $(0, 1)$ because it does not
contain its limits at the end points; the open interval will be more
convenient for this example going forward.
</div>
...
its simplest form, it's just an application of the chain rule and
definition of conditional probability, where if $\Pr[B] \neq 0,$
$$
...
inverse cdf, $F^{-1}_Z:(0, 1) \rightarrow \textrm{range}(Z).$ nFor a
...
## A.10 The central limit theorem {.unnumbered}

The *central limit theorem* characterizes the behavior of averages of
independent random variables and explains the centrality of the normal
distribution in statistics.  Throughout this section, we will suppose
that we have an infinite sequence of random variables $X_0, X_1,
\ldots$ that are *independent and identically distributed* (i.i.d.)
with a finite expectation, so that for all $n \in \mathbb{N},$
$$
\mu = \mathbb{E}[X_n]
$$
With the $X$ sequence, we can define a new
sequence of variables $Y_0, Y_1, \ldots$ to be the running averages of
the $X$s, 
$$
Y_n = \textrm{mean}(X_{0:n}) = \frac{1}{n} \sum_{i = 0}^n X_n.
$$


### The weak law of large numbers

The *weak law of large numbers* says that the running averages converge
in probability to $\mu,$
$$
\lim_{n \rightarrow \infty} Y_n = \mu,
$$
where to $converge in probability$ means
$$
\lim_{n \rightarrow \infty}
\Pr\!\left[ \left| Y_n - \mu \right| < \epsilon \right] = 1.
$$

### The strong law of large numbers

The *strong law of large numbers* extends the weak law of large
numbers to guarantee that the sequence *converges almost surely*
to $\mu,$ which requires
$$
\Pr[\lim_{n \rightarrow \infty} Y_n = \mu] = 1.
$$


### The central limit theorem

For the central limit theorem, we further assume that our i.i.d.\
variables $X_n$ have a finite variance, which allows us to define
their standard deviation to be
$$
\sigma = \textrm{sd}[X_n].
$$

The *central limit theorem* (CLT) tells us that the sequence $Y_1, Y_2,
\ldots$ *converges in distribution*, which means that the cdfs
converge,
$$
\lim_{n \rightarrow \infty}
F_{Y_n}(y) = \Phi\!\left(\frac{y}{\sqrt{n} \cdot \sigma}),
$$
where $\Phi(u)$ is the conventional notation for the cdf of the
standard normal distribution, and hence $f(v) =
\Phi\!\left(\frac{y - \nu}{\tau}\right)$ is the cdf of the
$\textrm{normal}(\nu, \tau)$ distribution.


where $Z \sim \textrm{normal}().$
which when we unpack the $\sim$ notation gives us
$$
\lim_{n \rightarrow \infty}
p_{Y_n}(y) = 
\textrm{normal}\left(
...
As with other normal approximations, the result begins to hold for $n
\gg 10.$ For example, here is the distribution of $Y_n$ for the case
of a coin flip, with $p_{X_n}(x) = \textrm{bernoulli}(0.5),$ for which
we know $\textrm{sd}[X_n] = 0.5.$ The result is remarkable here
because the Bernoulli distribution only produces outcomes 0 and 1.  In
this case, we know that $\textrm{sd}[X_n] = 0.5.$


```{python}
M = 10_000 if DRAFT else 1_000_000
df = pd.DataFrame(columns = ['N', 'y'])
for N in [5, 10, 20, 40, 80, 160]:
    y = [np.mean(u)/N for u in np.random.binomial(N, 0.5, size=M)]
    dfN = pd.DataFrame({'N': N, 'y': y})
    df = pd.concat([df, dfN], ignore_index = True)
mydraw(pn.ggplot(df, pn.aes(x = 'y'))
...
@roberts2004, @rosenthal2006, and @geyer2011.
...
In general, Monte Carlo methods use random algorithms to solve
deterministic integrals in high dimensions.


## C.1 Discrete-time stochastic processes {.unnumbered}

A *discrete-time stochastic process* is a countable sequence of random
variables $X = X^{(0)}, X^{(1)}, \ldots.$ For the purposes of this
book, we can restrict attention to real-valued variables on a state
space $\Omega,$ so that $X^{(n)}:\Omega \rightarrow \mathbb{R}^N.$ In
a stochastic process, the codomain $\mathbb{R}^N$ of the variables is
called the *state space.*

In the simplest non-trivial case, $N = 1$ so that $X^{(n)}(\omega) \in
\mathbb{R}$ and the entire sequence is an infinite-dimensional random
vector $X \in \mathbb{R}^{\mathbb{N}}.$



## Markov chains

A *Markov chain* is a discrete-time stochastic process, the evolution
of which is governed by an initial distribution on the state space and
a transition kernel governing the generation of the next state given
the previous state.

More formally, a Markov chain is determined by its initial
distribution with cdf
$$
F_{X^{(0)}}(x) = \Pr[X^{(0) \leq x],
$$
and and a memoryless transition measure, so that
$$
F_{X^{(m + 1)}}(x^{(m + 1)} \mid x) =
\Pr[X^{(m + 1)} \leq x^{ \mid X^{(m)} = x].
$$
and
$$
q
= p_{X^{(i + 1)} \mid X^{(i)}}(u, v)
= p_{X^{(j + 1)} \mid X^{(j)}}(u, v).
$$
