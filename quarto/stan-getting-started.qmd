...
```{python}
log_model = csp.CmdStanModel(stan_file = '../stan/iris-petals-log.stan')
...
plot =  pn.ggplot(df, pn.aes(x='log_petal_width', y='log_petal_length', color='species'))
plot = plot +  pn.geom_point()
plot = plot +  pn.labs(x = "petal width (log cm)", y = "petal length (log cm)")
for a, b in zip(alpha_draws[0:20], beta_draws[0:20]):
    plot = plot + pn.geom_abline(intercept = a, slope = b,
                                 alpha = 0.5, size = 0.2)
mydraw(
...
The varying slopes and varying intercepts are sometimes called _random
effects_ as opposed to our previous _fixed effects_ that work for all
items in the data set.

In Stan, all we need to do is ensure that we encode the species
as an indicator 1, 2, and 3 (which we will order in terms of
increasing size).  Then we will take that indicator in as an
array of integers, declare slopes and intercepts equal to the number
of groups, and then fit as usual.

```{.stan filename='../stan/iris-petals-random-effect.stan'}
